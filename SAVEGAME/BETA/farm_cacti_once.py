clear()
farm_cacti_once_optimized()
def farm_cacti_once_optimized():
	ws = get_world_size()
	ws_range = range(ws)
	l = []
	...
	for x in ws_range:
		...
		for y in ws_range:
			...
			if measure(West) and measure(West) > measure():
				...
				# quick_print(l)
				# l[x-1][y], l[x][y] = measure(), measure(West)
			if measure(South) and measure(South) > measure():
				swap(South)
				# l[x][y-1] = l[x][y]
				l[x][y-1], l[x][y] = l[x][y], l[x][y-1]
				# l[x][y-1], l[x][y] = measure(), measure(South)
			if measure(West) and measure(West) > measure():
				swap(West)
				# l[x-1][y] = l[x][y]
				l[x-1][y], l[x][y] = l[x][y], l[x-1][y]
