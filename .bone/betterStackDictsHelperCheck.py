...
def x2y3(mov=move, extend=False):
    if (get_pos_x(), get_pos_y()) != (2, 3):
        [] = [[],[]]
    mov(North)
    return x2y4

def x2y4(mov=move, extend=False):
    if (get_pos_x(), get_pos_y()) != (2, 4):
        [] = [[],[]]
    if extend or not mov(North):
        mov(West)
        mov(West)
...
def x3y7(mov=move, extend=False):
    if (get_pos_x(), get_pos_y()) != (3, 7):
        [] = [[],[]]
    mov(East)
    return x4y7

def x4y7(mov=move, extend=False):
    if (get_pos_x(), get_pos_y()) != (4, 7):
        [] = [[],[]]
    if extend or not mov(East):
        mov(North)
        mov(North)
...
def x7y6(mov=move, extend=False):
    if (get_pos_x(), get_pos_y()) != (7, 6):
        [] = [[],[]]
    mov(South)
    return x7y5

def x7y5(mov=move, extend=False):
    if (get_pos_x(), get_pos_y()) != (7, 5):
        [] = [[],[]]
    if extend or not mov(South):
        mov(East)
        mov(East)
...
def x6y2(mov=move, extend=False):
    if (get_pos_x(), get_pos_y()) != (6, 2):
        [] = [[],[]]
    mov(West)
    return x5y2

def x5y2(mov=move, extend=False):
    if (get_pos_x(), get_pos_y()) != (5, 2):
        [] = [[],[]]
    if extend or not mov(West):
        mov(South)
        mov(South)
